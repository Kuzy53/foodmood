...
const generateRow = (data: any, onDelete: (id: string) => void, setActive: (data: any) => void, activeEl: any) => {
  return data.map((row: any) => (
    <Table.Tr  key={row?.id || row?.name} onClick={() => setActive(row)} style={{background: activeEl?.id === row.id ? '#f3f4f5' : '', cursor: 'pointer'}}>
      ...
      <Table.Td>
        <Badge size={'md'} color={row.isAvailable ? 'green' : 'pink'} autoContrast >
          <Flex justify={'center'} align={'center'}>
            {row.isAvailable ? <IconCheck width={'17px'}/> : <IconX width={'17px'}/>}
          </Flex>
        </Badge>
      </Table.Td>
    </Table.Tr>
...
function Dishes() {
  ...
  const handleDelete = async (id: string) => {
    await removeItem('dishes/' + id)
    await fetchData('dishes')
    setActiveEl(null)
    form.reset()
  }
  ...
  const handleCreate = async (values: any) => {
    ...
    if(activeEl){
      ...
      res = await updateItem('dishes/' + activeEl.id, {
        dish: {
          ...values,
          price: +values.price,
      ...
      res = await createItem('dishes/dish', {
        dish: {
          ...values,
          price: +values.price,
          weight: +values.weight,
